# Mission 02 — Information Architecture
...
- **Lane (owns):** `app routing + sitemap` — only create/edit files here to stay collision-free.
- **Depends on:** Mission 01
- **Parallel-safe with:** Mission 03, Mission 04, Mission 05, Mission 06
- **Suggested wave:** 1
...
Define the new site structure. Use `rebuild/02-url-map.json` and `rebuild/02-new-sitemap.md`.
Plan redirects for high-value old URLs. Output: final sitemap + URL structure.
...
- Navigation items: 5
- Footer items: 5
- URL map in `rebuild/02-url-map.json`
- Sitemap in `rebuild/02-new-sitemap.md`
## Bundle References
